...
export class RippleHelper extends RippleHelperCommon {
	...
	private performHold() {
		...
		const size = tnsView.getActualSize();
		const holdanim = UIView.alloc().initWithFrame(CGRectMake(0, 0, size.width, size.height));
	...
	apply() {
		...
		if (outTnsView instanceof View && outTnsView.ios) {
			this.tapFn = (args: TouchGestureEventData) => {
				const tnsView = this.tnsView.get();
				if (tnsView) {
					if (this.holding && args.action === 'move' && (args.getX() < 0 || args.getX() > tnsView.getActualSize().width || args.getY() < 0 || args.getY() > tnsView.getActualSize().height)) {
						this.holding = false;
						this.performRipple(args.getX(), args.getY());
					} else if (args.action === 'down' && !this.holding) {
						this.holding = true;
						this.performHold();
					} else if (this.holding && (args.action === 'up' || args.action === 'cancel')) {
						this.holding = false;
						this.performRipple(args.getX(), args.getY());
					}
				}
			...
			outTnsView.on('touch', this.tapFn);
			return true;
		}
		return false;
	}
